$(document).ready(function(){
	var produto=[];
	var quantidade=[];
			
			/*valores[0]=1;
			valores[2]=0;
			valores[10]=2;
			var resultado=[];
			sessionStorage.setItem("valores", JSON.stringify(valores));
			var val = JSON.parse(sessionStorage.getItem("valores")); 
			console.log(val);
			console.log(val.length);
			for (var i = 0; i < val.length; i++) {
				if(val[i]!=null){
					resultado[i] = val[i];
					console.log("t"+i);
				}
			}*/
			
			
	$("div#tabela_produtos input").change(function(){
		if(produto[$(this).attr("id")]==null){
			var id=$(this).attr("id");
			var valor=$(this).val();
			produto[id]=id;
			quantidade[id]=valor;
			sessionStorage.setItem("produto", JSON.stringify(produto));
			sessionStorage.setItem("quantidade", JSON.stringify(quantidade));
					
			$.post("seleciona_itens.php", {"id":id}, function(v){
							
			});
					
					//var input="";
					//var div = $("#recebe").html();
					
					//input+=""+div+"<div id='item"+t+"'><input type='text' readonly='readonly' value='"+val+"'/><input type='number' name='"+t+"'/><button type='bitton' name='deletar' value='"+t+"'>deletar</button><br /></div>";
					
					//$("#recebe").html(input);
		}
		else{
			var id=$(this).attr("id");
			var valor=$(this).val();
			if(valor!=0){
				$("input[name='"+t+"']").val($(this).val());
			}
			else{
				produto[id]=null;
				sessionStorage.setItem("produto", JSON.stringify(produto));
				$("item"+id).remove();
			}
		}
	});
	$("input[name='deletar']").click(function(){
		var valor=$(this).val();
		produto[valor]=null;
		sessionStorage.setItem("produto", JSON.stringify(valores));
		$("item"+val).remove();
	});
});