			...
			for (var i = 0; i < item.length; i++) {
				if(item[i]!=null){
					...
					$.post("seleciona_itens.php", {"id":id}, function(g){
						console.log("y");
							input+=""+recebe+"<div id='item"+id+"'><input type='text' class='form-control' readonly='readonly' value='"+g+"'/><input type='number' name='"+id+"' value='"+quant[i]+""/><button type='button' name='deletar' class="btn btn-danger" value='"+id+"'>Deletar</button><br /></div>";		
						console.log(input);
					});			
				}
			}
	...
	function carrinho(){
		$("div#tabela_produtos button").click(function(){
				...
				if(produto[$(this).val()]==null){
					...
					$.post("seleciona_itens.php", {"id":id}, function(g){
						console.log("y");
							input+=""+div+"<div id='item"+id+"'><input type='text' class='form-control' readonly='readonly' value='"+g+"'/><input type='number' name='"+id+"'/><button type='button' name='deletar' class='btn btn-danger' value='"+id+"'>Deletar</button><br /></div>";			
