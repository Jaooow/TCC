				...
				<?php
					...
					if($id2==0){
						...
						while($linha=mysqli_fetch_assoc($res)){
							...
							<button onclick="aparece_negociacao('.$linha["id_usuario"].', 0)" class="btn btn-dark" type="button" data-toggle="collapse" data-target="#collapseExample'.$linha["id_usuario"].'" aria-expanded="false" aria-controls="collapseExample"> Negociação com '.$linha["nome_usuario"].'</button>
									<br />
						...
						while($linha=mysqli_fetch_assoc($res)){
							...
							<button onclick="aparece_negociacao('.$linha["id_usuario"].', 1)" class="btn btn-dark"> Negociação com '.$linha["nome_usuario"].'</button>';
						}
						echo $dados;
					}
				?>
